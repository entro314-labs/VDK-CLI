...
describe('VDKHubClient', () => {
  ...
  describe('Blueprint Synchronization', () => {
    it('should sync blueprints successfully', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith(
        expect.stringContaining(
          '/api/cli/sync/blueprints?since=2023-12-01T00%3A00%3A00Z&limit=50&category=frontend'
        ),
        expect.objectContaining({
          method: 'GET',
        })
    ...
    it('should handle sync failures gracefully', async () => {
      ...
      expect(consoleSpy.warn).toHaveBeenCalledWith(
        expect.stringContaining('Blueprint sync failed')
      );
    });
  ...
  describe('Telemetry Collection', () => {
    ...
    it('should handle telemetry failures gracefully', async () => {
      ...
      expect(consoleSpy.warn).toHaveBeenCalledWith(
        expect.stringContaining('Usage telemetry failed')
      );
    });
  ...
  describe('Community Operations', () => {
    ...
    it('should handle search failures gracefully', async () => {
      ...
      expect(consoleSpy.warn).toHaveBeenCalledWith(
        expect.stringContaining('Community blueprint search failed')
      );
    });
  ...
  describe('Authentication Management', () => {
    ...
    it('should save auth token', async () => {
      ...
      expect(fs.writeFile).toHaveBeenCalledWith(
        expect.stringContaining('.vdk-hub-auth'),
        'new-token-456',
        {
          mode: 0o600,
        }
    ...
    it('should handle auth token file errors gracefully', async () => {
      ...
      expect(consoleSpy.warn).toHaveBeenCalledWith(
        expect.stringContaining('Failed to save auth token')
      );
    });
  });
  ...
  describe('HTTP Client and Error Handling', () => {
    ...
    it('should handle 429 rate limit errors', async () => {
      ...
      await expect(hubClient.makeRequest('/test', { skipRetry: true })).rejects.toThrow(
        'Rate limit exceeded'
      );
    });
