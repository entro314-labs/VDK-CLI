...
describe('Migration System', () => {
  ...
  describe('MigrationDetector', () => {
    it('should detect Claude Code contexts', async () => {
      ...
      const claudeContext = contexts.find(ctx => ctx.type === 'claude-code')
    ...
    it('should calculate confidence correctly', async () => {
      ...
      const highConf = detector.calculateConfidence('claude-code', 'CLAUDE.md', 'mcp: server tool:', 'CLAUDE.md')
      expect(highConf).toBe('high')
      
      // Low confidence case - generic-ai with proper path and content
      const lowConf = detector.calculateConfidence('generic-ai', 'context.md', 'This is some AI context with assistant and prompt keywords', '.ai/context.md')
      expect(lowConf).toBe('low')
    })
  })
  
  describe('MigrationAdapter', () => {
    it('should adapt Claude Code contexts to VDK format', async () => {
      ...
      expect(adapted.platforms['claude-code']).toBeDefined()
      expect(adapted.platforms['claude-code'].compatible).toBe(true)
      expect(adapted.platforms['claude-code'].memory).toBe(true)
      expect(adapted.migration).toBeDefined()
      expect(adapted.migration.originalSource).toBe('Claude Code')
    })
    
    it('should generate appropriate blueprint IDs', async () => {
      const adapter = new MigrationAdapter()
      
      const context1 = { type: 'claude-code', fileName: 'CLAUDE.md' }
  ...
  describe('MigrationManager Integration', () => {
    ...
    it('should generate migration statistics', async () => {
      ...
      manager.results = {
        detected: [{ type: 'claude-code' }, { type: 'cursor' }],
        converted: [{ id: 'test1' }],
        generated: [{ id: 'test1', path: '/test' }],
        deployed: { successful: [{ name: 'Claude Code' }] },
        failed: [],
        skipped: []
      }
