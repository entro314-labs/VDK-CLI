...
export class UnifiedMigrateCommand extends BaseCommand {
  ...
  configureOptions(command) {
    return command
      ...
      .option('--type <type>', 'Migration type: auto, schema, context, or detect (default)', 'detect')
  ...
  displayMigrationResults(result, options) {
    ...
    if (result.type === 'schema') {
      ...
      if (result.ideIntegrations) this.logInfo(`Configured ${result.ideIntegrations} IDE integrations`)
      if (result.importedFiles?.length) this.logInfo(`Imported ${result.importedFiles.length} files`)
    }
