...
/**
 *  Claude Code CLI Adapter - AI SERVICE ADAPTATION ONLY
 *
 ...
 * -  memory hierarchy generation for AI context
...
export class ClaudeCodeAdapter extends RuleAdapter {
  ...
  /**
   *  Claude Code CLI adaptation - extends base RuleAdapter functionality
   */
  async adaptForClaude(rules, projectContext, platformConfig = {}, categoryFilter = null) {
    console.log('🧠 Generating Claude Code CLI memory hierarchy...');

    // First, get the base adaptation from parent class (includes platform config handling)
    // First, get the base adaptation from parent class
    const baseAdaptation = await super.adaptRules(
      rules,
      'claude-code',
      projectContext,
      platformConfig
    ...
    const enhancements = {
      ...
      agents: await this.generateAgents(rules, projectContext),
      conditionalRules: await this.generateConditionalRules(rules, projectContext),
      skills: await this.generateSkills(rules, projectContext),
    };
    ...
    // Add memory files (avoid duplicates with base adapter)
    const enhancedMemoryFiles = adapted.memoryHierarchy.filter(
      memory =>
        !adapted.files.some(file => path.basename(file.path || file) === path.basename(memory.path))
    );
    ...
    if (adapted.agents) {
      for (const agent of adapted.agents) {
        // Use manifest filename if available, otherwise name
        const filename = agent.manifest?.file || `${agent.name}.md`;
        adapted.files.push({
          path: path.join(this.claudeConfigPath, 'agents', filename),
    ...
    // Add conditional rules (v3.0)
    if (adapted.conditionalRules) {
      for (const rule of adapted.conditionalRules) {
        const filename = rule.manifest?.file || `${rule.name}.md`;
        adapted.files.push({
          path: path.join(this.claudeConfigPath, 'rules', filename),
    ...
    if (adapted.skills) {
      for (const skill of adapted.skills) {
        const filename = skill.manifest?.file || `${skill.name}.md`;
        adapted.files.push({
          path: path.join(this.claudeConfigPath, 'skills', filename),
...
- Use project-specific conventions`;
    ...
    for (const framework of frameworks) {
      //  framework matching with aliases
...
- Reference remote rules: ${technologyRules.length} rules available`;
  ...
  /**
   * Generate agents from v3.0 rules
   */
  async generateAgents(rules, _projectContext) {
    const agents = [];
    const agentRules = rules.filter(
      r => r.frontmatter?.type === 'agents' || r.frontmatter?.type === 'claude-agent'
    );

    for (const rule of agentRules) {
      // Validate agent requirements
      const content = rule.content || '';
      const manifest = rule.frontmatter;

      // Ensure agent has required sections if not structured
      let enrichedContent = content;
      if (!content.includes('## Role')) {
        enrichedContent = `# ${manifest.name}\n\n## Role\n${manifest.description || 'Specialized agent'}\n\n${content}`;
      }

      agents.push({
        name: rule.name || manifest.name,
  ...
  /**
   * Generate conditional rules from v3.0 specs
   */
  async generateConditionalRules(rules, _projectContext) {
    const conditionalRules = [];
    const ruleSpecs = rules.filter(
      r => r.frontmatter?.type === 'rules' || r.frontmatter?.type === 'claude-rule'
    );

    for (const rule of ruleSpecs) {
      const manifest = rule.frontmatter;

      // Construct file content with frontmatter for Claude to read (if it supports frontmatter-based rules)
      // or just pure markdown. Claude Code rules usually need to be in .claude/rules/ and matched by globs?
      // Actually v3.0 spec says conditional rules have `paths` in manifest.

      // We'll preserve specific frontmatter for the rule file if needed
  ...
  /**
   * Generate skills from v3.0 specs
   */
  async generateSkills(rules, _projectContext) {
    const skills = [];
    const skillSpecs = rules.filter(
      r => r.frontmatter?.type === 'skills' || r.frontmatter?.type === 'claude-skill'
    );

    for (const skill of skillSpecs) {
      skills.push({
        name: skill.name || skill.frontmatter?.name,
        content: skill.content,
        manifest: skill.frontmatter,
      });
    }
    return skills;
  }
  ...
  async detectPrimaryIDE(projectContext) {
    try {
      ...
      // Check for IDE-specific files and folders with platform detection
      const ideIndicators = {
...
*This file manages Claude Code CLI's integration with external tools and services*`;
   ...
   */
  async generateSlashCommands(_rules, projectContext, categoryFilter = null) {
    try {
