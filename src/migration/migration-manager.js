...
export class MigrationManager {
  ...
  async migrate(options = {}) {
    ...
    try {
      ...
      if (migrationContexts.length === 0) {
        ...
        console.log(
          chalk.gray('Tip: Run `vdk init` to generate fresh AI contexts for your project')
        );
        return this.results;
      }
      ...
      const adaptedContexts = await this.migrationAdapter.adaptContextsToVDK(migrationContexts, {
        projectData,
        techData,
      });
   ...
   */
  async deployToIntegrations(_generatedRules) {
...
${Object.entries(detectionsByType)
  .map(([type, count]) => `- **${type}**: ${count} contexts`)
  .join('\n')}
...
`;
  ...
  getStats() {
    return {
      ...
      skipped: this.results.skipped.length,
      successRate:
        this.results.detected.length > 0
